...
namespace MitoR
{
  ...
  Configuracao::Impl::~Impl()
  {
    if(arquivo.isOpen()) {
      ...
      //spdlog::info("Arquivo de configuração '{}', salvo com sucesso.", arquivo.fileName().toStdString());
      arquivo.close();
    }
  }
  void Configuracao::Impl::inicializar()
  {
  ...
  void Configuracao::Impl::destruirRecursos()
  {
    spdlog::info("Destruindo recursos...'{}'", arquivo.fileName().toStdString());
  }
  QMap<QString, std::shared_ptr<Configuracao::Impl>> Configuracao::Impl::configuracoes;

  Configuracao::Configuracao(const QString idConfigurador)
  {
    Impl::inicializar();
    auto it = pThis->configuracoes.find(idConfigurador);
    if(it == pThis->configuracoes.end()) {
  ...
  Configuracao::Configuracao(const QString idConfigurador, const QFileInfo nomeArquivo) : Configuracao(idConfigurador)
  {
  ...
  bool Configuracao::array(const QString &strPointerPath, QByteArray *arr) const
  {
  ...
  bool Configuracao::array(const QString &strPointerPath, const QByteArray &arr)
  {
    const auto path = Json::json_pointer(strPointerPath.toStdString().c_str());
  ...
  bool Configuracao::integer(const QString &strPointerPath, int *valor) const
  {
    const auto path = Json::json_pointer(strPointerPath.toStdString().c_str());
    if(pThis->pJson->contains(path)) {
  ...
  bool Configuracao::integer(const QString &strPointerPath, const int valor)
  {
    const auto path = Json::json_pointer(strPointerPath.toStdString().c_str());
    pThis->pJson->operator[](path) = valor;
    return true;
  }
  bool Configuracao::string(const QString &strPointerPath, QString *string) const
  {
    const auto path = Json::json_pointer(strPointerPath.toStdString().c_str());
    if(pThis->pJson->contains(path)) {
      std::string str;
  ...
  bool Configuracao::string(const QString &strPointerPath, const QString &str)
  {
    const auto path = Json::json_pointer(strPointerPath.toStdString().c_str());
